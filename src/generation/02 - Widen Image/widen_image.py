...
def _calculate_target_dimensions(source_width: int, source_height: int) -> Dimensions:
...
def _outpaint_image() -> str:
    ...
    _logger.header("Identifying Source Image and Output")

    # Verify our source image exists
    source_image_path = Path(_args.output) / _config.data.paths.result_dir / \
        _config.data.paths.temp_dir / _config.data.paths.outputs.stage_01
    ...
    final_output_path = Path(_args.output) / _config.data.paths.result_dir / \
        _config.data.paths.temp_dir / _config.data.paths.outputs.stage_02
    final_output_path.parent.mkdir(parents=True, exist_ok=True)
    final_result.save(final_output_path, quality=_config.data.generation.outpaint.save_quality)
